...
import { processTestSubmission } from "../services/tests/processTestSubmission.service.js";
...
//     // ---------------------------------------------------------
//     // 1. Resolve and authorize the student
//     // ---------------------------------------------------------
//     const studentResult = await resolveStudent({
//       userId,
//       studentId,
//     });

//     if (!studentResult.success) {
//       return res.status(studentResult.statusCode).json({
...
//     ) {
//       return res.status(400).json({
//         error: "noOfQuestions must be a positive integer.",
//       });
//     }
...
//   } catch (error) {
//     console.error("SAVE TEST SCORE CONTROLLER ERROR:", error);
...
export async function fetchStudentScores(
  ...
  try {
    ...
    const getEffectiveAverage = (subject) => {
      const hasExam = subject.exam.count > 0;
      const hasTest = subject.test.count > 0;
    ...
    const totalTestScore =
      formatted.reduce(
        (sum, subject) =>
          sum +
          Number(
            subject.test.total || 0
          ),
        0
      );
    ...
    if (bestSubject) {
      insights.push(
        `${bestSubject.subjectName} is the subject with the highest score and an average of ${getEffectiveAverage(
          bestSubject
        ).toFixed(
          1
        )}%. You can also improve more on it`
      );
    }

    if (weakestSubject) {
      insights.push(
        `${weakestSubject.subjectName} requires more attention with an average score of ${getEffectiveAverage(
          weakestSubject
        ).toFixed(
          1
        )}%`
    ...
    return res.status(200).json({
      ...
      summary: {
        ...
        overallTestAverage:
          Number(
            overallTestAverage.toFixed(2)
          ),

        overallExamAverage:
          Number(
            overallExamAverage.toFixed(2)
          ),
